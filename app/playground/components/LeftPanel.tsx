    ...
    try {
      const response = await fetch(
        `/api/search?term=${encodeURIComponent(
          query
        )}&platform=${platformFilter}&limit=${limit}`
      );
        ...
        try {
          ...
          if (data.screenshots && data.screenshots.length > 0) {
            ...
            const validShots = shots.filter(
              (s: string) => s && s.startsWith("data:image")
            );
            if (validShots.length > 0) {
              setDetails((prev) => ({ ...prev, screenshots: validShots }));
            }
          ...
          if (
            data.description &&
            data.description.length > (app.description?.length || 0)
          ) {
    ...
    <div className="w-full md:w-[70%] h-full overflow-y-auto bg-white border-r border-slate-200 p-6 md:p-10 pb-32">
      <div className="mb-8">
        ...
        <p className="text-slate-500">
          Generate high-conversion mobile video ads in seconds.
        </p>
      </div>
