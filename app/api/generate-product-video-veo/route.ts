...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      `Starting Product Video Generation (${scenes.length} Scenes) with Ratio: ${selectedRatio}...`
    );
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      const prompt = `
        ${scene.name} (${i * 8}-${(i + 1) * 8}s): Product showcase video for ${
        details.productName
      } (${details.productType}).
        Action: ${scene.script}
        Style: ${concept.description}, ${
        details.tone
      }, high quality product cinematography.
    ...
    console.error("Veo Product Generation Error:", error);
    return NextResponse.json(
